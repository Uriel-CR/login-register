...
<body>
    ...
    $sql_creditos_aprobados = "SELECT SUM(creditos_aprobados_parcial_$parcial)
    FROM calificaciones_resumen
    WHERE id_grupo = ?
    AND id_periodo = ?";

    $sql_creditos_reprobados = "SELECT SUM(creditos_reprobados_parcial_$parcial)
    FROM calificaciones_resumen
    WHERE id_grupo = ?
    AND id_periodo = ?";
